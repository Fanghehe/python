...
if url==[]:
	...
	#prompt for input
	print('%s%s&param=true&fromHtml=true&userAgentForLogin=0&%s'%(host_url,login_uri,query_args))
	...
	#do the POST
	#print('%s%s&aram=true&fromHtml=true&userAgentForLogin=0&%s'%(host_url,login_uri,query_args))x
	req=urllib2.Request('%s%s&param=true&fromHtml=true&userAgentForLogin=0&%s'%(host_url,login_uri,query_args))
	#print '%s'%(query_args)
